...
require_once '../../controller/SeanceS.php';
...
// Récupérer la liste des cours pour alimenter la liste déroulante
$cours = $coursC->listCours();

// Vérification si le formulaire est soumis
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (
        isset($_POST["id_matiereseance"]) &&
        isset($_POST["date_seance"]) &&
        isset($_POST["heure_d"]) &&
        isset($_POST["heure_f"]) &&
        isset($_POST["description"])
    ) {
        if (
            !empty($_POST["id_matiereseance"]) &&
            !empty($_POST["date_seance"]) &&
            !empty($_POST["heure_d"]) &&
            !empty($_POST["heure_f"]) &&
            !empty($_POST["description"])
        ) {
            // Validation et création de la séance
            $idMatiereseance = $_POST['id_matiereseance'];
            $dateSeance = $_POST['date_seance'];
            $heureD = $_POST['heure_d'];
            $heureF = $_POST['heure_f'];
            $description = $_POST['description'];

            // Vérifier si le cours existe (sécurité supplémentaire)
            $coursExiste = $coursC->affichercours($idMatiereseance);
            if ($coursExiste) {
                $seance = new Seance(
                    null,
                    $idMatiereseance,
...
<div class="container">
        ...
        <?php foreach ($cours as $matiere) { ?>
            ...
            <option value="<?= htmlspecialchars($matiere['id_matiere']); ?>">
                <?= htmlspecialchars($matiere['id_matiere']); ?>
            </option>
        <?php } ?>
    </select>
